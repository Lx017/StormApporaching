...
namespace MxM
{
    ...
    *********************************************************************************************/
    ...
    public partial class MxMAnimator : MonoBehaviour
    {
        ...
        private bool m_shouldForceInstantRotation = false;
        private float m_forcedAngle = 0f;
    ...
    protected virtual void OnEnable()
        ...
        *********************************************************************************************/
        protected virtual void OnAnimatorMove()
        {
            ...
            if (m_shouldForceInstantRotation)
            {
                // 1) 强制设置旋转
                var finalPos = m_animationRoot.position; // 不动位置
                var finalRot = Quaternion.Euler(0f, m_forcedAngle, 0f);
                m_animationRoot.SetPositionAndRotation(finalPos, finalRot);

                // 2) 下次帧就不再强制，避免持续覆盖
                m_shouldForceInstantRotation = false;
            }
        ...
        public void ForceInstantRotation(float targetAngle)
        {
            m_shouldForceInstantRotation = true;
            m_forcedAngle = targetAngle;
        }
