...
namespace JUTPS.AnimatorStateMachineBehaviours
{
    public class UseCurrentMeleeWeapon : StateMachineBehaviour
    {
        ...
        override public void OnStateUpdate(Animator animator, AnimatorStateInfo stateInfo, int layerIndex)
        {
            ...
            var rightWeapon = Controller.MeleeWeaponInUseRightHand;
            if (rightWeapon)
            {
                var col = rightWeapon.GetComponent<CapsuleCollider>();

                if (col != null) { col.enabled = (stateInfo.normalizedTime > StartUsing && stateInfo.normalizedTime < StopUsing); }
                else { 
                    var vol = rightWeapon.GetComponent<BoxCollider>();
                    if (vol != null) { vol.enabled = (stateInfo.normalizedTime > StartUsing && stateInfo.normalizedTime < StopUsing); }
                }
            }
