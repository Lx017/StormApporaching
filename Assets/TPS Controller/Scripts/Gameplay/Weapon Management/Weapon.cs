...
namespace JUTPS.WeaponSystem
{
	...
	public class Weapon : JUHoldableItem
	{
		...
		{
			float FRScale = PlayerStats.fireRateScale;
			// >>> Fire Rate Control
			if (CanUseItem == false)
			{
				CurrentFireRateToShoot += Time.deltaTime;
				if (CurrentFireRateToShoot >= Fire_Rate/FRScale)
				{
		...
		public void Shot()
		{
			...
			if (FireMode != Weapon.WeaponFireMode.Shotgun)
			{
				...
				float oldX = ShootDirection.x;
				float oldY = ShootDirection.y;
				float oldZ = ShootDirection.z;
				var BulletRotationPrecision = ShootDirection;

				bool audioType = false;
				for(int i =0;i<PlayerStats.numBulletPerShot;i++){
					// Generate Random Direction
					BulletRotationPrecision.x = Random.Range(-ShotErrorProbability / 2, ShotErrorProbability / 2) + oldX;
					BulletRotationPrecision.y = Random.Range(-ShotErrorProbability / 2, ShotErrorProbability / 2) + oldY;
					BulletRotationPrecision.z = Random.Range(-ShotErrorProbability / 2, ShotErrorProbability / 2) + oldZ;
