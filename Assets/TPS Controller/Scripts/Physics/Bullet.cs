...
namespace JUTPS.WeaponSystem
{
	...
	public class Bullet : MonoBehaviour
	{
		...
		[Header("4455 added:")]
		public float hitBack = 3f;
		public float fallRate = 100f;
		public float explosionRange = 5f;
		public float explosionDamage = 0.1f;
		public float explosionImpactForce = 3f;
		...
		void Start()
		{
			...
			{
				rb.useGravity = false;//this does not work!
			}
		...
		{
			fallingVelocity += fallRate * Time.deltaTime;
			rb.linearVelocity = transform.forward * BulletVelocity + Vector3.down * fallingVelocity * Time.deltaTime;
		}
		...
		void OnCollisionEnter(Collision col)
		{
			...
			{
				float RealDamage = BulletDamage * PlayerStats.damageScale;
				...
				{
					EnemyStats.DoAOE(explosionCenter, explosionRange, explosionDamage*PlayerStats.damageScale,explosionImpactForce);
				}
				...
				if (ImpactAddForce)
				{
					...
					{
						other.AddForce(transform.forward * hitBack, ForceMode.Impulse);
					}
				}
				...
				//Debug.Log("Bullet Collided with: " + col.gameObject.name + ", tag: " + col.gameObject.tag + ", layer: " + col.gameObject.layer + ", damage: " + RealDamage);
				DestroyBullet(0);
				// if (Ricochet)
				// {
